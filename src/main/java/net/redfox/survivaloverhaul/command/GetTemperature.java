...
public class GetTemperature {
  public GetTemperature(CommandDispatcher<CommandSourceStack> dispatcher) {
    dispatcher.register(Commands.literal("temperature")
        .then(Commands.literal("get").executes((command) -> getTemperature(command.getSource()))));
  }

  private int getTemperature(CommandSourceStack context) {
    if (context.getPlayer() != null)
      context.getPlayer().sendSystemMessage(
          Component.literal("Your current temperature is: " + ClientTemperatureData.getPlayerTemperature()));
