...
public class ConfigUtil {
  ...
  public static void init() {
    ...
    passive_hunger = ModCommonConfigs.PASSIVE_EXHAUSTION_RATE.get().floatValue() / 300f; // Convert to exhaustion per
                                                                                         // tick
    StringParsingUtil.fillMap(EGG_MULTIPLIERS, ModCommonConfigs.EGG_TIME_MODIFIERS.get(), Difficulty::byName,
        Double::parseDouble);
    StringParsingUtil.fillMap(GLOBAL_HUNGER_MULTIPLIERS, ModCommonConfigs.GLOBAL_HUNGER_MULTIPLIERS.get(),
        Difficulty::byName, Double::parseDouble);
    StringParsingUtil.fillMap(BREEDING_MULTIPLIERS, ModCommonConfigs.BREEDING_COOLDOWN_MULTIPLIERS.get(),
        Difficulty::byName, Double::parseDouble);
    StringParsingUtil.fillMap(BABY_AGE_MULTIPLIERS, ModCommonConfigs.BABY_GROWTH_MODIFIERS.get(), Difficulty::byName,
        Double::parseDouble);
    StringParsingUtil.fillMap(BONEMEAL_LOWER_MULTIPLIERS, ModCommonConfigs.BONE_MEAL_LOWER_LIMIT.get(),
        Difficulty::byName, Integer::parseInt);
    StringParsingUtil.fillMap(BONEMEAL_UPPER_MULTIPLIERS, ModCommonConfigs.BONE_MEAL_UPPER_LIMIT.get(),
        Difficulty::byName, Integer::parseInt);
  }
